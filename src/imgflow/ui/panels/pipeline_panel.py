"""Pipeline node'larını sıralı liste olarak gösteren panel; sürükle-bırak ile yeniden sıralanabilir."""
...
class PipelinePanel(QListWidget):
    ...
    def refresh(self) -> None:
        ...
        for index, node_id in enumerate(self.pipeline.order, start=1):
            ...
            self.addItem(f"{index}. {node_id}  ({node.op_id})")
    ...
    def _on_current_changed(self, current, previous) -> None:
        if current is None:
            return
        self.node_selected.emit(current.data(_NODE_ID_ROLE))
